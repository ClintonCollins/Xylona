...
Page structure uses the shared utilities in `design-tokens.css`: `.xy-page-content` for the responsive content well (16px/24px padding, widening to 24px/32px above 1024px), `.xy-page-header` for the title-and-actions band, `.xy-section-title` for second-level headings, `.xy-section-overline` for uppercase section labels, `.xy-num` for comparable numbers, and `.xy-banner-warning` / `.xy-banner-negative` / `.xy-banner-info` for tinted inline banners (never solid `bg-warning` or `bg-negative`). Use `src/components/shared/PageHeader.vue` for standard page titles, subtitles, and actions; reserve direct `.xy-page-header` markup for context-specific headers that do not fit that structure.

Tables put row actions in a sticky `xy-col-actions` column (`classes` and `headerClasses` on the column) wrapped in `.xy-row-actions` dense round buttons. Show a list's `EmptyState` only when it is neither loading nor failed; a failed load shows `.xy-banner-negative` with Retry. Toasts go through `src/api/notifications.ts` (`notifySuccess`, `notifyError`, `notifyWarning`, `notifyInfo`), and absolute times through `src/utils/format-timestamp.ts`. Badges with `positive`, `negative`, `warning` or `info` render as status tints; use `color="grey-8"` for labels that are not status.
