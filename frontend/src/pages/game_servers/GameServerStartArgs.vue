...
import { askLifecycleConfirmation, buildLifecycleConfirmation } from './server-list-actions'
...
async function onlinePlayerCount(): Promise<number> {
  try {
    const response = await GetXylonaClient().queryGameServer(
      create(QueryGameServerRequestSchema, { serverId: gameServerId.value }),
    )
    return response.queryInfo ? (queryInfoPlayerSnapshot(response.queryInfo)?.playerCount ?? 0) : 0
  } catch (unknownError: unknown) {
    // Like the server bar, an unknown player count restarts without asking.
