        ...
        <q-card id="fileUploaderCard" class="fileUploaderDialogCard q-px-lg"
            ...
            <q-card-section>
                ...
                <q-toolbar class="bg-blue-grey-10 q-py-sm">
                    ...
                    <q-btn v-if="uploader.queuedFilesCount > 0 && uploader.canUpload"
                ...
                <div class="file-uploader">
                    <q-list separator>
                        <q-item v-if="uploader.files.size <= maxNumberOfFilesToDisplay"
                    ...
                    <div v-if="uploader.files.size > maxNumberOfFilesToDisplay"
                        ...
                        <div>You've selected <span class="text-bold">{{ uploader.files.size }}</span> files and/or
                            directories. For
                            performance reasons, we only display up to <span
                                    class="text-bold">{{ maxNumberOfFilesToDisplay }}</span>.
                        </div>
                        <div class="text-info text-bold">You can still upload your files by clicking the upload
                            button.
                        </div>
                    </div>
                </div>
...
class FileUploader {
    ...
    async addFiles(files: FileSystemFileEntry[]) {
        await Promise.all(files.map((fileEntry: FileSystemFileEntry) => {
    ...
    addFile(file: File, fileEntry: FileSystemFileEntry) {
        ...
        const updatedFile = new File([file], getUploaderFileRelativePath(fileEntry), {type: file.type})
...
async function processDirectoryEntry(webkitDirEntry: FileSystemDirectoryEntry): Promise<FileSystemFileEntry[]> {
    return new Promise((resolveDirectory) => {
        webkitDirEntry.createReader().readEntries(async (entries: FileSystemEntry[]) => {
            const allPromises = entries.map((entry: FileSystemEntry) => {
                if (entry.isFile) {
...
async function handleDropEvent(event: DragEvent) {
    ...
    if (event.dataTransfer.items) {
        ...
        const directoryProcessors: (Promise<FileSystemFileEntry[]> | Promise<FileSystemFileEntry>)[] = items.map((item: DataTransferItem) => { // All the items should finally map to a Promise<File>
