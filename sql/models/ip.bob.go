...
var Ips = sqlite.NewTablex[*IP, IPSlice, *IPSetter]("", "ip", buildIPColumns("ip"), ipScanMapper)
...
func ipScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*IP, error)) {
	...
	for i, col := range cols {
		switch col {
		case "address":
			targets = append(targets, target{i, func(o *IP) any { return &o.Address }})
		case "usable":
			targets = append(targets, target{i, func(o *IP) any { return &o.Usable }})
		case "external":
			targets = append(targets, target{i, func(o *IP) any { return &o.External }})
		case "automatically_added":
			targets = append(targets, target{i, func(o *IP) any { return &o.AutomaticallyAdded }})
		case "node_id":
			targets = append(targets, target{i, func(o *IP) any { return &o.NodeID }})
		}
	}
...
func buildIPWhere[Q sqlite.Filterable](cols ipColumns) ipWhere[Q] {
	return ipWhere[Q]{
		...
		R:                  ipWhereR[Q]{cols: cols},
	}
}

// ipWhereR holds the relationship-based filters of
// ipWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type ipWhereR[Q sqlite.Filterable] struct {
	cols ipColumns
}
...
func (w ipWhereR[Q]) HasGameServers(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServers.NameExpr()),
		sm.Where(GameServers.Columns.IP.EQ(w.cols.Address)),
		sm.Where(GameServers.Columns.NodeID.EQ(w.cols.NodeID)),
	)
	q.Apply(filters...)
...
func (w ipWhereR[Q]) HasNode(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
...
// NULL-tolerant scan buffer for ipScanMapperNullable:
...
type ipPreloadBuf struct {
	Address            null.Val[string]
	Usable             null.Val[bool]
	External           null.Val[bool]
	AutomaticallyAdded null.Val[bool]
	NodeID             null.Val[string]
}

// ipScanMapperNullable maps the preloaded ip
...
func ipScanMapperNullable(prefix string) scan.Mapper[*IP] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*IP, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *ipPreloadBuf) any
		}
		targets := make([]target, 0, 5)
		for i, col := range cols {
			...
			switch name {
			case "address":
				targets = append(targets, target{i, func(b *ipPreloadBuf) any { return &b.Address }})
			case "usable":
				targets = append(targets, target{i, func(b *ipPreloadBuf) any { return &b.Usable }})
			case "external":
				targets = append(targets, target{i, func(b *ipPreloadBuf) any { return &b.External }})
			case "automatically_added":
				targets = append(targets, target{i, func(b *ipPreloadBuf) any { return &b.AutomaticallyAdded }})
			case "node_id":
				targets = append(targets, target{i, func(b *ipPreloadBuf) any { return &b.NodeID }})
			}
		}
		...
		buf := new(ipPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*IP, error) {
				buf := link.(*ipPreloadBuf)
				...
				if !(buf.Address.IsValue()) &&
					!(buf.Usable.IsValue()) &&
					!(buf.External.IsValue()) &&
					!(buf.AutomaticallyAdded.IsValue()) &&
					!(buf.NodeID.IsValue()) {
					return nil, nil
				}
				...
				o := new(IP)
				if buf.Address.IsValue() {
					o.Address = buf.Address.MustGet()
				}
				if buf.Usable.IsValue() {
					o.Usable = buf.Usable.MustGet()
				}
				if buf.External.IsValue() {
					o.External = buf.External.MustGet()
				}
				if buf.AutomaticallyAdded.IsValue() {
					o.AutomaticallyAdded = buf.AutomaticallyAdded.MustGet()
				}
