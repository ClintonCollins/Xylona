...
var Migrations = sqlite.NewTablex[*Migration, MigrationSlice, *MigrationSetter]("", "migrations", buildMigrationColumns("migrations"), migrationScanMapper)
...
func migrationScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Migration, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *Migration) any
	}
	targets := make([]target, 0, 2)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *Migration) any { return &o.ID }})
		case "applied_at":
			targets = append(targets, target{i, func(o *Migration) any { return &o.AppliedAt }})
		}
	}
...
func (o MigrationSlice) copyMatchingRows(from ...*Migration) {
	fromByPK := make(map[string]*Migration, len(from))
