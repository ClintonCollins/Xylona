...
var Users = sqlite.NewTablex[*User, UserSlice, *UserSetter]("", "user", buildUserColumns("user"), userScanMapper)
...
func userScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*User, error)) {
	...
	for i, col := range cols {
		...
		case "password_hash":
			targets = append(targets, target{i, func(o *User) any { return &o.PasswordHash }})
		case "super_user":
			targets = append(targets, target{i, func(o *User) any { return &o.SuperUser }})
		case "last_login_at":
			targets = append(targets, target{i, func(o *User) any { return &o.LastLoginAt }})
		case "created_at":
...
type userWhereR[Q sqlite.Filterable] struct {
	cols userColumns
}
...
func (w userWhereR[Q]) HasAlertHistories(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(AlertHistories.NameExpr()),
		sm.Where(AlertHistories.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
...
func (w userWhereR[Q]) HasAlertRules(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(AlertRules.NameExpr()),
		sm.Where(AlertRules.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}
...
func (w userWhereR[Q]) HasGameServers(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServers.NameExpr()),
		sm.Where(GameServers.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasUpdatedByUserGameServerMinecraftMaps filters parents that have a matching UpdatedByUserGameServerMinecraftMaps using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to GameServerMinecraftMaps).
func (w userWhereR[Q]) HasUpdatedByUserGameServerMinecraftMaps(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServerMinecraftMaps.NameExpr()),
		sm.Where(GameServerMinecraftMaps.Columns.UpdatedByUserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasUpdatedByUserGameServerPalworldMaps filters parents that have a matching UpdatedByUserGameServerPalworldMaps using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to GameServerPalworldMaps).
func (w userWhereR[Q]) HasUpdatedByUserGameServerPalworldMaps(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServerPalworldMaps.NameExpr()),
		sm.Where(GameServerPalworldMaps.Columns.UpdatedByUserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasUpdatedByUserGameServerSecrets filters parents that have a matching UpdatedByUserGameServerSecrets using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to GameServerSecrets).
func (w userWhereR[Q]) HasUpdatedByUserGameServerSecrets(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServerSecrets.NameExpr()),
		sm.Where(GameServerSecrets.Columns.UpdatedByUserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasUpdatedByUserGameServerSevenDaysToDieMaps filters parents that have a matching UpdatedByUserGameServerSevenDaysToDieMaps using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to GameServerSevenDaysToDieMaps).
func (w userWhereR[Q]) HasUpdatedByUserGameServerSevenDaysToDieMaps(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServerSevenDaysToDieMaps.NameExpr()),
		sm.Where(GameServerSevenDaysToDieMaps.Columns.UpdatedByUserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasCreatedByUserGameServerSevenDaysToDieMapShares filters parents that have a matching CreatedByUserGameServerSevenDaysToDieMapShares using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to GameServerSevenDaysToDieMapShares).
func (w userWhereR[Q]) HasCreatedByUserGameServerSevenDaysToDieMapShares(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServerSevenDaysToDieMapShares.NameExpr()),
		sm.Where(GameServerSevenDaysToDieMapShares.Columns.CreatedByUserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasGameServerStatusPage filters parents that have a matching GameServerStatusPage using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to GameServerStatusPages).
func (w userWhereR[Q]) HasGameServerStatusPage(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(GameServerStatusPages.NameExpr()),
		sm.Where(GameServerStatusPages.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasNotificationChannels filters parents that have a matching NotificationChannels using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to NotificationChannels).
func (w userWhereR[Q]) HasNotificationChannels(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(NotificationChannels.NameExpr()),
		sm.Where(NotificationChannels.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasCreatedByScheduledTasks filters parents that have a matching CreatedByScheduledTasks using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to ScheduledTasks).
func (w userWhereR[Q]) HasCreatedByScheduledTasks(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(ScheduledTasks.NameExpr()),
		sm.Where(ScheduledTasks.Columns.CreatedBy.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasRequestedByUserSystemUpdateJobs filters parents that have a matching RequestedByUserSystemUpdateJobs using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to SystemUpdateJobs).
func (w userWhereR[Q]) HasRequestedByUserSystemUpdateJobs(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(SystemUpdateJobs.NameExpr()),
		sm.Where(SystemUpdateJobs.Columns.RequestedByUserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasGrantedByUserRoleAssignments filters parents that have a matching GrantedByUserRoleAssignments using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to UserRoleAssignments).
func (w userWhereR[Q]) HasGrantedByUserRoleAssignments(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(UserRoleAssignments.NameExpr()),
		sm.Where(UserRoleAssignments.Columns.GrantedBy.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}
...
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to UserRoleAssignments).
func (w userWhereR[Q]) HasUserRoleAssignments(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(UserRoleAssignments.NameExpr()),
		sm.Where(UserRoleAssignments.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}

// HasUserSessions filters parents that have a matching UserSessions using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to UserSessions).
func (w userWhereR[Q]) HasUserSessions(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := sqlite.Select(
		sm.Columns(sqlite.Raw("1")),
		sm.From(UserSessions.NameExpr()),
		sm.Where(UserSessions.Columns.UserID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: sqlite.Exists(q)}
}
...
type userPreloadBuf struct {
	...
	PasswordHash null.Val[string]
	SuperUser    null.Val[bool]
	LastLoginAt  null.Val[time.Time]
...
func userScanMapperNullable(prefix string) scan.Mapper[*User] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*User, error)) {
		...
		for i, col := range cols {
			...
			case "password_hash":
				targets = append(targets, target{i, func(b *userPreloadBuf) any { return &b.PasswordHash }})
			case "super_user":
				targets = append(targets, target{i, func(b *userPreloadBuf) any { return &b.SuperUser }})
			case "last_login_at":
				targets = append(targets, target{i, func(b *userPreloadBuf) any { return &b.LastLoginAt }})
			case "created_at":
		...
		return func(row *scan.Row) (any, error) {
				...
				if !(buf.ID.IsValue()) &&
					...
					!(buf.PasswordHash.IsValue()) &&
					!(buf.SuperUser.IsValue()) &&
					!(buf.LastLoginAt.IsValue()) &&
				...
				if buf.PasswordHash.IsValue() {
					o.PasswordHash = buf.PasswordHash.MustGet()
				}
				if buf.SuperUser.IsValue() {
					o.SuperUser = buf.SuperUser.MustGet()
				}
				if buf.LastLoginAt.IsValue() {
					o.LastLoginAt = buf.LastLoginAt.MustGet()
				}
